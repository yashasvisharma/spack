...
def fetch_remote_package_versions(pkg):
    """Try to find remote versions of a package.

    Args:
        pkg (Package): package instance to spider for versions.

    This uses ``list_url`` and any other URLs described in the package
    file.
    """
    if not pkg.all_urls:
        raise VersionFetchError(
            ("Cannot fetch versions for package %s "
             "because it does not define any URLs.") % pkg.name)

    try:
        return find_versions_of_archive(
            pkg.all_urls, pkg.list_url, pkg.list_depth)
